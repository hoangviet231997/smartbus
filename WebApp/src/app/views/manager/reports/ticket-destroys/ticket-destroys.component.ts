...
import { TicketDestroyView, TicketDestroyForm } from '../../../../api/models';
...
@Component({
  selector: 'app-ticket-destroys',
  templateUrl: './ticket-destroys.component.html',
  styleUrls: ['./ticket-destroys.component.css']
})
export class TicketDestroysComponent implements OnInit {
  
  @ViewChild('showImageTicketDestroy') public showImageTicketDestroy: ModalDirective;

  public bsRangeValue: Date[];
  public maxDate: Date;
  public ticket_destroys: any = [];
  public isLoading = false;
  public permissions:any[] = [];
  public showImage = '';
  public isViewHistoryTicketDestroy : any = 0;
  ...
  clickShowImageTicketDestroy(image: any){
    this.showImage = image;
    this.showImageTicketDestroy.show();
  }

  getDataTicketsDestroy(){

    this.isLoading = true;

    if( this.isViewHistoryTicketDestroy == 0 ){
    ...
    if(this.isViewHistoryTicketDestroy == 1){
    ...
    this.spinner.show();
    this.apiReports.managerReportsViewTicketDestroy({
      ...
      data => {
        this.ticket_destroys = data;
        this.spinner.hide();
    });
  }

  acceptTicketDestroy(id: number, type: string){
    ...
    swal({
      ...
      if (result.value) {
        this.spinner.show();
        this.apiReports.managerReportsAcceptTicketDestroy({
          id: id,
          type: type
        }).subscribe(
          res => {
            this.getDataTicketsDestroy();
            this.spinner.hide();
  ...
  showViewHistoryTicketDestroy(){

    this.isViewHistoryTicketDestroy = 1;
    this.bsRangeValue = null;
    this.selectedTypeAccept = null;
    this.ticket_destroys = [];
    this.isLoading = false;
  }

  callBack(){

    this.isViewHistoryTicketDestroy = 0;
    this.bsRangeValue = null;
    this.selectedTypeAccept = null;
    this.ticket_destroys = [];
    this.isLoading = false;
